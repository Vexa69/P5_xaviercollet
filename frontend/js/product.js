...
function addToBasket(data) {
	...
	for (let i = 0; i < basket.length; i++) {
		if (
			data._id === basket[i]._id &&
			data.price === basket[i].price &&
			data.selectedLense === basket[i].selectedLense
		) {
	...
	if (isThisItemExist === false) {
		...
		existingItem.selectedQuantity =
			parseInt(existingItem.selectedQuantity, 10) + parseInt(data.selectedQuantity, 10);
		localStorage.setItem("basket", JSON.stringify(basket));
	}
	manageBasketDisplay();
